...
class Agent(gym.Env, list):
    ...
    def step(self, action_value, eps = 0.0):
        ...
        for i in range(8):
            if self.state[i] == -10:
                avg_aoi += self.state[i+8] * 11
            else:
                avg_aoi += self.state[i+8] * (self.state[i]+1) 
            if self.state[i] > 9:
                self.reward -= 10
            #if self.state[i] > 0:
                #self.reward += self.state[i+8]*(10 - self.state[i]-1)
            elif self.state[i] > 0 and self.state[i] <= 9 :
                self.reward += self.state[i+8]*(10 - self.state[i]-1)
            #elif self.state[i] <= 5 and self.state[i] > 0:
            #    self.reward += self.state[i+10]*(10 - self.state[i]-1 + 5)
            elif self.state[i] < 0:
                self.reward += self.state[i+8]*(-1)
        """        
        #print(self.ts)
        #print(self.reward)
